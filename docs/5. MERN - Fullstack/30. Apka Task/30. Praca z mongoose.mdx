...
---
Poznamy pracę z biblioteką `mongoose` na przykładzie aplikacji `Task`,
w której zadania o strukturze tytuł i opis będą zapisywane do bazy danych.
...
- to szablon dokumentów w kolekcji (określa strukturę danych w MongoDB i mówi,
jakie pola powinny znaleźć się dokumencie
- wskazuje, jakie pola będą przechowywane i jakie mają typy.
- w przeciwieństwie do interfejsu, `Schema` wpływa na bazę danych – decyduje,
jakie dane będą zapisywane.
...
- teraz TypeScript podpowiada nam, że właściwości obiektu muszą być określonego typu
(np. `title` musi być `string`)
...
- dzięki temu dziedziczeniu `Task`
    ...
    - ma dostęp do metod Mongoose:
        - **instancyjnych** (wywoływanych na obiekcie np. task.save()
            ...
            - `deleteOne()` - usuwa jeden  dokument z bazy
