...
## Przykłady
- Przygotujmy bazę danych `cartoon` i kolekcję `cartoonCharacters`.
- pracujemy w `Compass`
    - łączymy się z Mongo
    - w połączeniu (lewa strona okienka) jest znak `+` dodający bazę danych:
        - baza `cartoon`
        - kolekcja `cartoonCharacters`
...
Zmiana zapytania tak, aby wyświetlało się tylko imię bohatera i jednego przyhaciela:
```
db.cartoonCharacters.find(
  { name: "Shrek" },
